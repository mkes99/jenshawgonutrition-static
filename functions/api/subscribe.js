 ...
 * - upserts to Mailchimp audience + applies subscribe tag
 * - sends admin email via Resend (optional)
 *
 * Env:
 * - RECAPTCHA_SECRET
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const hp = (isForm ? input.get("hp_name") : input.hp_name) || "";
    if (String(hp).trim()) {
      return json({ ok: true, message: "Thanks! You’re subscribed." }, 200);
    }
    ...
    if (!email) return json({ ok: false, message: "Please enter your email address." }, 400);
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
      return json({ ok: false, message: "Please enter a valid email address." }, 400);
    }

    await verifyRecaptchaV3({ env, token, action: "subscribe" });

    const listId = str(env.MAILCHIMP_AUDIENCE_ID);
    const tag = str(env.MAILCHIMP_SUBSCRIBE_TAG);
    if (!listId) return json({ ok: false, message: "Mailchimp audience is not configured." }, 500);
    if (!tag) return json({ ok: false, message: "Mailchimp subscribe tag is not configured." }, 500);

    await upsertMailchimpMember({ env, listId, email, tag });
...
async function sendResendAdminEmail({ env, subject, html, replyTo }) {
  const apiKey = env.RESEND_API_KEY;
  const emailTo = env.EMAIL_TO;
  const emailFrom = env.EMAIL_FROM;

  if (!apiKey || !emailTo || !emailFrom) return;
